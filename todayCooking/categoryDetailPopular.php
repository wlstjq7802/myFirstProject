<?php
    ...
    if($type=="전체" || $situation=="전체" || $ingredient=="전체" || $method=="전체"){
        ...
        if($type=="전체" && $situation!="전체" && $ingredient!="전체" && $method!="전체"){
            ...
            ingredientCategory = '$ingredient' AND methodCategory = '$method' 
            ORDER BY likeCount desc, recipeNum desc";
        } 
        // 상황별만 전체일 경우
        ...
        // 재료별만 전체일 경우
            ...
            $recipeSql = "SELECT * FROM recipe WHERE typeCategory = '$type' AND 
            situationCategory = '$situation' AND methodCategory = '$method' 
            ORDER BY likeCount desc, recipeNum desc";
        }
        // 방법별만 전체일 경우
        else if($type!="전체" && $situation!="전체" && $ingredient!="전체" && $method=="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE typeCategory = '$type' AND 
            situationCategory = '$situation' AND ingredientCategory = '$ingredient' 
            ORDER BY likeCount desc, recipeNum desc";
        }
        // 종류별, 상황별 전체일 경우
        else if($type=="전체" && $situation=="전체" && $ingredient!="전체" && $method!="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE ingredientCategory = '$ingredient' AND 
            methodCategory = '$method' ORDER BY likeCount desc, recipeNum desc";
        }
        // 종류별, 재료별 전체일 경우
        else if($type=="전체" && $situation!="전체" && $ingredient=="전체" && $method!="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE situationCategory = '$situation' AND
        ...
        // 종류별, 방법별 전체일 경우
            ...
            $recipeSql = "SELECT * FROM recipe WHERE situationCategory = '$situation' AND 
            ingredientCategory = '$ingredient' ORDER BY likeCount desc, recipeNum desc";
        }
        // 상황별, 재료별 전체일 경우
        else if($type!="전체" && $situation=="전체" && $ingredient=="전체" && $method!="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE typeCategory = '$type' AND  
        ...
        // 재료별, 방법별 전체일 경우
            ...
            $recipeSql = "SELECT * FROM recipe WHERE typeCategory = '$type' AND 
            situationCategory = '$situation' ORDER BY likeCount desc, recipeNum desc";
        }
         // 종류별, 상황별, 재료별이 전체일 경우
         else if($type=="전체" && $situation=="전체" && $ingredient=="전체" && $method!="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE methodCategory = '$method' 
            ORDER BY likeCount desc, recipeNum desc";
        } 
        // 종류별, 상황별, 방법별이 전체일 경우
        else if($type=="전체" && $situation=="전체" && $ingredient!="전체" && $method=="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE ingredientCategory = '$ingredient' 
            ORDER BY likeCount desc, recipeNum desc";
        } 
        // 종류별, 재료별, 방법별이 전체일 경우
        else if($type=="전체" && $situation!="전체" && $ingredient=="전체" && $method=="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE situationCategory = '$situation' 
            ORDER BY likeCount desc, recipeNum desc";
        } 
        // 상황별, 재료별, 방법별이 전체일 경우
        else if($type!="전체" && $situation=="전체" && $ingredient=="전체" && $method=="전체"){
            $recipeSql = "SELECT * FROM recipe WHERE typeCategory = '$type' 
            ORDER BY likeCount desc, recipeNum desc";
        } 
        // 전부 전체일 경우
        else if($type=="전체" && $situation=="전체" && $ingredient=="전체" && $method=="전체"){
            $recipeSql = "SELECT * FROM recipe ORDER BY likeCount desc, recipeNum desc";
        } 
